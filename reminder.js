...
function combineAllList() {
    for (var key in bedData) {
        for (var entry in bedData[key]) {
            if (bedData[key][entry].checkOutDate != null && bedData[key][entry].checkOutDate == getTodayDate()) {
                waitingList.push(bedData[key][entry]);
            }
        }
    }
}
...
function populateTableContent() {
    ...
    if (waitingList.length == 0) {
        bedTableContent += "<tr><td colspan='8'>No patient pending check out today.</td></tr>"
    }
}